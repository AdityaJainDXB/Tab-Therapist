...
function displayTabs() {
  ...
  const container =
    document.getElementById("tabsList");
  ...
  if (allTabs.length === 0) {
    ...
    container.innerHTML =
      `<div class="empty">No tabs found.</div>`;
...
async function saveTab(tab){
    const existing = 
    savedTabs.find (t => t.url === tab.url);
    if(existing) {
        return;
    }
...
function displaySavedTabs() {
    const container = 
    document.getElementById("savedTabsList");
    container.innerHTML = ""

    if(savedTabs.length === 0 ){
        container.innerHTML = 
        '<div class="empty">No saved tabs.</div>';
...
for (const tab of allTabs){
    ...
    for (const [name,domain] of Object.entries(categories)){
